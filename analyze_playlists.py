...
def create_model_comparison_plot(df):
    ...
    model_song_counts = (
        df.groupby(["model", "song_id"]).size().reset_index(name="count")
    )
...
def create_model_diversity_plot(model_songs):
    ...
    model_unique_songs = {
        model: len(set(songs)) for model, songs in model_songs.items()
    }

    # Sort models by number of unique songs
    sorted_models = dict(
        sorted(model_unique_songs.items(), key=lambda x: x[1], reverse=True)
    )
...
def get_model_statistics(df):
    ...
    for model in df["model"].unique():
        ...
        top_songs = []
        for _, row in (
            model_df.groupby(["song", "artist"])["song"]
            .count()
        ...
        top_artists = []
        for _, row in (
            model_df.groupby(["artist"])["artist"]
            .count()
            .reset_index(name="count")
            .sort_values("count", ascending=False)
            .head(5)
        ...
        stats.append(
            {
                ...
                "diversity_ratio": (
                    round(unique_songs / total_songs, 2) if total_songs > 0 else 0
                ),
                "top_songs": top_songs,
